...
def test_function_call_with_keyword_arguments():
    assert [10, 7, 3, 4] == demo(10, 7)
    assert [10, 20, 3, 4] == demo(10, 20)
    assert [10, 20, 30, 4] == demo(10, 20, 30)
    assert [5, 20, 3, 4] == demo(first = 5, second=20)
    assert [30, 20, 3, 4] == demo(second=20, first=30)
    assert [10, 30, 3, 4] == demo(first=10, second=30)
    assert [1, 30, 3, 4] == demo(1, 30)
...
- I have learnt that args can take 1 or more parameters and it returns a tuple representation
- also kwargs will take the arguments which are assigned type and returns the dictionary of those pairs
- modification of strings using replace function
"""
