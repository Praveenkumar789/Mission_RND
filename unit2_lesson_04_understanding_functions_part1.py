...
def get_word_with_least_vowels(input):
    ...
    for name in input:
        number = 0
        number = name.count("a") + name.count("e") + name.count("i") + name.count("o") + name.count("u")
        num_of_vowels[name] = number
    for name in num_of_vowels.keys():
        if num_of_vowels[name] == min(num_of_vowels.values()):
            return name
...
def get_min_max_words(input):
    ...
    t = ()
    for word in input:
        min_max[word] = len(word)
    for key in min_max.keys():
        if min_max[key] == min(min_max.values()):
            minimum = key
        if min_max[key] == max(min_max.values()):
            maximum = key
...
- I have learnt that callable functions checks whether the parameter passed ino it is callable or not its return type is a boolean
- I have learnt that __doc__ will return the diocuments of its parameter(function)
- I have also learnt abut the function's object reference
"""