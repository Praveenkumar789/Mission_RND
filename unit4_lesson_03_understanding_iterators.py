...
def test_find_builtins_that_work_on_iterables():
    funcs = ['abs()',	'delattr()','hash()','memoryview()', 'set([1,2,3])', 'all([2>0, True])	', 'dict()', 'help()', 'min()', 'setattr()','any()', 'dir()', 'hex()', 'next()', 'slice()', 'ascii()', 'divmod()', 'id()', 'object()', 'sorted()', 'bin()', 'enumerate()', 'input()', 'oct()', 'staticmethod()', 'bool()', 'eval()', 'int()', 'open()', 'str()', 'breakpoint()', 'exec()', 'isinstance()', 'ord()', 	'sum()', 'bytearray()', 'filter()', 'issubclass()', 'pow()', 'super()', 'bytes()', 'float()', 'iter()', 'print()', 'tuple()',
              'callable()', 'format()','len()', 'property()', 'type()','chr()', 'frozenset()', 'list()', 'range()', 'vars()','classmethod()', 'getattr()','locals()', 'repr()', 'zip()', 'compile()', 'globals()', 'map()', 'reversed()', '__import__()', 'complex()', 'hasattr()', 'max()', 'round()']
    assert 69 == len(funcs)
...
def my_map(func, iterable):
    result = []
    Objects = iterable.__iter__()
    while True:
        try:
            result.append(func(Objects.__next__()))
        except StopIteration:
            return result
...
- I have learnt that list range and also tuple uses iter() internally for iterating over the elements(objects)
- string.join also works using this iter()
- for loop works by iterating over the objects(elements) 
"""