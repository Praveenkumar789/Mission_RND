...
def custom_sort(input):
    try:
        input = sorted(sorted(input, key=lambda x: x[2]), key=lambda x: x[1], reverse=True)
    #type error ('none') or value error ("empty list")
    except:
        pass
    return input
...
def max_vowels(words):
    ...
    try :
        ...
        L =[]
        for word in words:
            Alpha[word] = get_vowels_count(word)
        for key in Alpha.keys():
            if Alpha[key] == max(Alpha.values()):
                L.append(key)
        return min(L)
    except TypeError:
        pass
