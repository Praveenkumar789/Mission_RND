...
def get_reverse_mapping():
    ...
    """
    return {string.ascii_lowercase[i]: string.ascii_lowercase[25 -i] for i in range(26)}
...
def get_lower_to_upper_dict():
    ...
    """
    return {string.ascii_lowercase[i] : string.ascii_uppercase[i] for i in range(26)}
...
def get_encoding_dict():
    ...
    """
    return {(string.ascii_letters[i]):(i+1 if i<26 else i - 25) for i in range(52)}
...
def get_months():
    ...
    """
    return {list(calendar.month_abbr)[i] : i for i in range(1,13)}
