...
def test_file_tell():
    ...
    while True:
        ...
        if not line:
            break
    assert [5, 10, 17, 23,  29, 31, 31] == tells
...
def test_file_readlines_tell():
    ...
    for line in f.readlines():
        tells.append(f.tell())
    assert [31, 31, 31, 31, 31, 31] == tells
...
def test_file_iteration_tell():
    ...
    for line in f:
        tells.append(f.tell())
    print(tells)
    assert [5, 10, 17, 23, 29, 31] == tells # is there really no difference between readlines and iteration?
...
-I have learnt about the file modes and file methods , binary files are interpreted byte wise while txt files are interpreted characterwise
- readline method reads line to line while readlines method will return a list of lines while read method reads the entire file
- seek will have two arguments 1st one tells about the index and second one is from what(0 -begin, 1- current pos, 2- end), tell method tells the current position of file
"""