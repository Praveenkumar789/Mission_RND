...
def get_odds_desc(count):
    ...
    """
    ODD_LIST =list(range(1, count*2, 2))
...
def get_multiples_desc(number, count):
    ...
    lists lesson.
    """
    multiples = list(range(number, number*count+1, number))
...
def ascii_distance(first, second):
    ...
    F = ord(first)
    S = ord(second)
    return abs(F - S) # use 2 builtin functions to get your work done.
...
def get_binary(number):
    ...
    B =  bin(number)
    bin_no = B[2:]
    return bin_no # use a builtin and slice the result ...
