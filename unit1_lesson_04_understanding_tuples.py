...
def min_max(input):
    ...
    maximum = max(input)
    L = [minimum, maximum]
    return tuple(L) # write a line of code to return containing min and max
...
-If a tuple contains lists or other tuples as its objects, we can access them By double index
- tuples are mainly used in areas where we want constants rather variables
"""
